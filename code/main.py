...
modelsvc = 'FLASK_CHABOT_MODEL_API_SVC'
...
def mbqa(text, qa_text, modelip):
    headers = {'Content-Type': 'application/json'}
    data = {"text": text, 'qa_text': qa_text}
    qa_res = requests.post(modelip + '/mbqa', headers=headers, data=json.dumps(data))
    return qa_res.json()
...
def get_bot_response(_debug=False, _demoText=''):
    ...
    modelip = 'http://' + os.environ[modelsvc + '_SERVICE_HOST'] + ':' + os.environ[modelsvc + '_PORT'].split(':')[2]
    res_lstm = requests.post(modelip + '/clf', headers=headers, data=json.dumps({"text": userText}))
    res = res_lstm.json()
    dompb = res["probability"]

    if not '魔速方塊' in userText:
        if float(res["probability"]) > 0.8:
            result = res["result"]
        else:
            result = res["result"]
            return ("<font color='gray'>Probability：{}，Domain：{}".format(res["probability"], res["result"]) + "</font><br>" + \
                        "我不了解您的問題，請專人為您服務")
    else:
        result = '魔速方塊'
    ...
    if result == '合約查詢':        
        result_proj = requests.post(modelip + '/proj', headers=headers, data=json.dumps({"text": userText}))        
        result_int = result_proj.json()
    ...
    if result == '帳務查詢':
        result_bill = requests.post(modelip + '/bill', headers=headers, data=json.dumps({"text": userText}))        
        result_int = result_bill.json()
    ...
    if result == '魔速方塊':
        ...
        if not os.path.isfile('./cum_status/' + remote_addr + '/cube_tmp.json'):
            if userText == '申辦' or userText == '1':
                ...
                my_result = mbqa(str(userText), mb_data[0][0], modelip)                
        ...
        result_mb = requests.post(modelip + '/mb', headers=headers, data=json.dumps({"text": userText}))        
        result_int = result_mb.json()
